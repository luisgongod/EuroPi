...
class AnalogueInput(AnalogueReader):
    ...
    def __init__(self, pin, calibration_values = INPUT_CALIBRATION_VALUES, min_voltage=MIN_INPUT_VOLTAGE, max_voltage=MAX_INPUT_VOLTAGE):
        ...
        # Because mux_dev has up to 8 analog inputs, each input has a calibration set of values. 
        # Calibration values are stored in the calibration_values_dev.py generated by the calibration_dev.py script.
    ...
    def read_voltage(self, samples=None):
        ...
        if len(self._gradients) == 2:
            cv = 10 * (reading / self._calibration_values[-1])
        else:
